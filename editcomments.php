...
if (isset($_POST['edit'])) {
    ...
    $result = mysql_query("UPDATE artykuly_comments SET name='$name', email='$email', url='$url', comment='$comment' WHERE id='$id' LIMIT 1") or print ("Can't update comment.<br />" . $result . "<br />" . mysql_error());
    if ($result != false) {
        print "<p>Zedytowa³eœ pomyœlnie komentarz!</p>";
    }
}

if(isset($_POST['delete'])) {
$id = (int)$_POST['id'];
     $result = mysql_query("DELETE FROM artykuly_comments WHERE id='$id' LIMIT 1") or print ("Can't delete comment.<br />" . $result . "<br />" . mysql_error());
     if ($result != false) {
...
$result = mysql_query ("SELECT * FROM artykuly_comments WHERE id='$_GET[id]'") or print ("Can't select comment.<br />" . mysql_error());
...
$result = mysql_query("SELECT entry AS get_group FROM artykuly_comments GROUP BY get_group DESC LIMIT 10") or print ("Can't select comments.<br />" . $result . "<br />" . mysql_error());
...
while($row = mysql_fetch_array($result)) {
    ...
    $result2 = mysql_query("SELECT timestamp, title FROM artykuly WHERE id='$get_group'");
    ...
    $result3 = mysql_query("SELECT * FROM artykuly_comments WHERE entry='$get_group' ORDER BY timestamp DESC");
